# ------------------------------------------------------------------------------
# Code adapted from https://github.com/hendrycks/robustness for performing 
# simple texture/noise operations.
# ------------------------------------------------------------------------------
...
d['Shot Noise'] = speckle_noise
d['Gaussian Blur'] = gaussian_blur
d['Spatter'] = spatter
d['Saturate'] = saturate

def corrupt_img(img, texture_name, texture_level=5):
    while texture_level > 0:
        if texture_level > 4:
            curr_level = 5
        else:
            curr_level = texture_level
        texture_level -= curr_level
        texture = lambda clean_img: d[texture_name](clean_img, curr_level)
        img = np.uint8(texture(img))
